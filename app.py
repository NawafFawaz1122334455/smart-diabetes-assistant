...
TRANSLATIONS = {
    'ar': {
        ...
        'exercise_image': "صورة صفحة التمارين",
        'water_image': "صورة صفحة حاسبة المياه",
        'select_image_to_upload': "اختر الصورة المراد تحديثها",
        'upload_page_image_button': "تحديث الصورة",
        'image_updated_success': "تم تحديث صورة الصفحة بنجاح!",
        'image_updated_failed': "فشل تحديث صورة الصفحة:",
        'no_image_uploaded': "الرجاء تحميل ملف صورة."
    },
    'en': {
...
def init_session_state():
    ...
    # حالة لتخزين روابط الصور مؤقتاً
...
def show_home_page():
    ...
    # *** التعديل: جلب رابط الصورة من قاعدة البيانات ***
...
def show_admin_page():
    ...
    if admin_password == SECRET_CODE:
        ...
        st.markdown("---")
        # عرض نموذج إضافة المنتجات
        show_add_product_form()
        st.markdown("---")
        # عرض نموذج تعديل/حذف المنتجات
        ...
        if submit_button:
            if uploaded_image:
                with st.spinner(t('updating_image_spinner')):
                    # prefix هو اسم الصفحة لتمييز الصور في التخزين
...
if st.session_state['user']:
    ...
    # تحديد الدالة المناسبة بناءً على الاختيار
    selected_function = page_options[page_name_translated]
    
    # تحديث حالة الصفحة لتعكس الاختيار الجديد 
    st.session_state['page'] = selected_function.__name__.replace('show_', '').replace('_page', '').capitalize()
    ...
    # هذا الزر يجب أن يكون Button عادي وليس Radio
    if st.sidebar.button(t('reset_password_button'), key='reset_pw_btn'):
        user_email = st.session_state['user'].email
        handle_password_reset(supabase, user_email) # استدعاء الدالة الجديدة
        
    # زر تسجيل الخروج
    if st.sidebar.button(t('logout'), key='logout_btn'):
        logout_user()

    # 4. استدعاء الدالة المختارة لعرض محتوى الصفحة
    selected_function()
