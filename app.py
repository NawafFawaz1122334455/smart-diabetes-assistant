...
def show_home_page():
    ...
    # رابط الصورة المحددة (شعار SMART DA)
    image_url = "https://cdn.pixabay.com/photo/2023/05/01/12/34/smart-da-logo-1234567_960_720.png" 
    
    # ضع هنا رابط الصورة الفعلي
    try:
        # عرض الصورة
        st.image(image_url, width=400, caption=get_text("welcome_title")) 
    except Exception as e:
        st.warning("⚠️ فشل تحميل الصورة. تحقق من الاتصال بالإنترنت.") 
        
    # نصوص ترحيبية
    st.markdown(f"**{get_text('welcome_msg_1')}**") # استخدم Bolding بدلاً من h3
